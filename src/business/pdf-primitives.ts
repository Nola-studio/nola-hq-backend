...
/** Fully-rounded capsule badge, e.g. "● Envoyée". Bullet is part of the rendered label, matching both HTML references (the "●" is literal content, not a CSS pseudo-element). Returns the rendered width. */
export function statusPill(doc: Doc, x: number, y: number, label: string, colors: { bg: string; text: string }): number {
  // A drawn dot, not a "●" (U+25CF) text character — confirmed absent from
  // the vendored Plus Jakarta Sans glyph set (it renders as a broken glyph;
  // only U+2022 "•" is covered, and even that renders smaller/higher than
  // the reference's flat status-dot). A real filled circle is font-independent.
  ...
  doc.fillColor(colors.text).text(text, dotCx + dotRadius + dotGap, y + 3, { characterSpacing: 0.4, lineBreak: false });
  return width;
}

/** Small rectangular chip (distinct from statusPill's full capsule) — used for the header's document-type badge and the footer's mini brand mark. */
export function chip(doc: Doc, x: number, y: number, label: string, colors: { bg: string; text: string }, radius = 4): number {
  ...
  doc.fillColor(colors.text).text(label, x + paddingX, y + 4, { characterSpacing: 1, lineBreak: false });
  return width;
}
...
/** Label-left / value-right stacked rows — meta-card-2's date/facture/statut block, and the totals breakdown. Returns the Y just past the last row. */
export function keyValueList(doc: Doc, x: number, y: number, width: number, rows: KeyValueRow[], lineHeight = 15): number {
  ...
  for (const r of rows) {
    doc.font(PDF_FONTS.regular).fontSize(10.5).fillColor(PDF_NEUTRALS.textMuted).text(r.label, x, cursorY, { width: labelWidth, lineBreak: false });
    doc
      .font(r.mono ? PDF_FONTS.mono : PDF_FONTS.semibold)
      .fontSize(r.mono ? 9.5 : 10.5)
      .fillColor(r.color ?? PDF_NEUTRALS.textMain)
      .text(r.value, x + labelWidth, cursorY, { width: valueWidth, align: 'right', lineBreak: false });
    cursorY += lineHeight;
  }
  return cursorY;
}
...
export function itemsTableHeader(doc: Doc, x: number, y: number, cols: ItemsTableColumns): number {
  ...
  let cx = x;
  doc.text(cols.descLabel.toUpperCase(), cx + 10, y + 9, { width: cols.descWidth - 10, characterSpacing: 0.4, lineBreak: false });
  cx += cols.descWidth;
  doc.text(cols.qtyLabel.toUpperCase(), cx, y + 9, { width: cols.qtyWidth, align: 'center', characterSpacing: 0.4, lineBreak: false });
  cx += cols.qtyWidth;
  doc.text(cols.unitLabel.toUpperCase(), cx, y + 9, { width: cols.unitWidth - 10, align: 'right', characterSpacing: 0.4, lineBreak: false });
  cx += cols.unitWidth;
  doc.text(cols.totalLabel.toUpperCase(), cx, y + 9, { width: cols.totalWidth - 10, align: 'right', characterSpacing: 0.4, lineBreak: false });
  return y + 26;
}

/** One item row. Returns the row's height so the caller can advance its cursor (and, in the pagination-aware caller, decide whether to break). */
export function itemsTableRow(doc: Doc, x: number, y: number, cols: ItemsTableColumns, item: ItemRow): number {
  ...
  doc.font(PDF_FONTS.semibold).fontSize(11).fillColor(PDF_NEUTRALS.textMain).text(item.description, x + 10, y + 8, { width: cols.descWidth - 20, lineBreak: false });
  if (item.sub) {
  ...
  doc.font(PDF_FONTS.semibold).fontSize(11).fillColor(PDF_NEUTRALS.textMain).text(item.qty, cx, y + rowHeight / 2 - 5.5, { width: cols.qtyWidth, align: 'center', lineBreak: false });
  cx += cols.qtyWidth;
  doc.font(PDF_FONTS.mono).fontSize(10.5).text(item.unitPrice, cx, y + rowHeight / 2 - 5, { width: cols.unitWidth - 10, align: 'right', lineBreak: false });
  cx += cols.unitWidth;
  doc.font(PDF_FONTS.monoBold).fontSize(10.5).text(item.total, cx, y + rowHeight / 2 - 5, { width: cols.totalWidth - 10, align: 'right', lineBreak: false });
...
export function totalsBlock(doc: Doc, spec: TotalsBlockSpec): number {
  ...
  doc.font(PDF_FONTS.semibold).fontSize(10.5).fillColor('#FFFFFF').text(spec.grandLabel, spec.x + 12, grandY + 13, { lineBreak: false });
  doc.font(PDF_FONTS.monoBold).fontSize(14.5).text(spec.grandValue, spec.x, grandY + 11, { width: spec.width - 12, align: 'right', lineBreak: false });
...
export function paymentBox(doc: Doc, spec: PaymentBoxSpec): void {
  ...
  for (const pill of spec.pills) {
    ...
    doc.fillColor(pill.text).text(pill.label, cx + 6, pillY + 4, { lineBreak: false });
    cx += w + 6;
  }
  if (spec.bodyText) {
...
export function signatureBlock(doc: Doc, y: number, cards: SignatureCard[], gap = 15): void {
  ...
  cards.forEach((card, i) => {
    ...
    if (card.kind === 'qr' && card.qrImage) {
      ...
      doc.font(PDF_FONTS.regular).fontSize(8).fillColor(PDF_NEUTRALS.textMuted).text(card.qrCaption ?? '', x, y + 62, { width: card.width, align: 'center', lineBreak: false });
      doc.font(PDF_FONTS.monoBold).fontSize(8.5).fillColor(PDF_NEUTRALS.textMain).text(card.qrToken ?? '', x, y + 74, { width: card.width, align: 'center', lineBreak: false });
    } else {
      doc
        .font(PDF_FONTS.regular)
        .fontSize(8.5)
        .fillColor(PDF_NEUTRALS.textLight);
      ...
      doc.text(stampLabel, stampX, y + 29, { width: stampWidth, align: 'center', lineBreak: false });
      doc.moveTo(x, y + 66).lineTo(x + card.width, y + 66).strokeColor(PDF_NEUTRALS.borderFocus).stroke();
      doc.font(PDF_FONTS.bold).fontSize(8.5).fillColor(PDF_NEUTRALS.textMuted).text(card.caption.toUpperCase(), x, y + 71, { width: card.width, align: 'center', characterSpacing: 0.5, lineBreak: false });
    }
  });
}
...
export function header(doc: Doc, spec: HeaderSpec): void {
  ...
  doc.font(PDF_FONTS.extrabold).fontSize(20).fillColor('#FFFFFF').text(spec.monogram, 50, 63, { width: 40, align: 'center', lineBreak: false });

  doc.font(PDF_FONTS.bold).fontSize(16).fillColor(PDF_NEUTRALS.textMain).text(spec.brandName, 102, 52, { width: 263, lineBreak: false });
  doc.font(PDF_FONTS.regular).fontSize(10.5).fillColor(PDF_NEUTRALS.textMuted).text(spec.brandSub, 102, 71, { width: 263, lineBreak: false });

  chip(doc, 545 - chipWidth(doc, spec.badge), 50, spec.badge.toUpperCase(), { bg: spec.theme.primaryLight, text: spec.theme.primary });
  doc.font(PDF_FONTS.bold).fontSize(15).fillColor(PDF_NEUTRALS.textMain).text(spec.title, 365, 68, { width: 180, align: 'right', lineBreak: false });
  doc.font(PDF_FONTS.mono).fontSize(10.5).fillColor(PDF_NEUTRALS.textMuted).text(spec.number, 365, 86, { width: 180, align: 'right', lineBreak: false });
...
export function metaCards(doc: Doc, y: number, spec: MetaCardsSpec): void {
  ...
  doc.font(PDF_FONTS.bold).fontSize(13.5).fillColor(PDF_NEUTRALS.textMain).text(spec.card1Name, x1 + 12, y + 26, { width: 261, lineBreak: false });
  ...
  if (spec.card2StatusPill) {
    ...
    doc.font(PDF_FONTS.regular).fontSize(10.5).fillColor(PDF_NEUTRALS.textMuted).text(lastLabel, x2 + 12, afterY, { width: 100, lineBreak: false });
    statusPill(doc, x2 + 12 + 90, afterY - 2, spec.card2StatusPill.label, { bg: spec.card2StatusPill.theme.badgeBg, text: spec.card2StatusPill.theme.badgeText });
  }
}
...
export function footer(doc: Doc, y: number, spec: FooterSpec): void {
  ...
  doc.text(spec.monogram, 50, textY + 2, { width: tagSize, align: 'center', lineBreak: false });
  doc.font(PDF_FONTS.regular).fontSize(9.5).fillColor(PDF_NEUTRALS.textMuted).text(spec.brandLine, 50 + tagSize + 5, textY, { width: 300 - tagSize - 5, lineBreak: false });
  doc.text(spec.rightText, 350, textY, { width: 195, align: 'right', lineBreak: false });
}
...
export function pageNumberStamp(doc: Doc, page: number, totalPages: number): void {
  doc.font(PDF_FONTS.mono).fontSize(8).fillColor(PDF_NEUTRALS.textLight).text(`Page ${page} / ${totalPages}`, 445, PAGE.height - 40, { width: 100, align: 'right', lineBreak: false });
}
