...
export interface MomoSummary {
  ...
  net_by_category: Record<string, { net_cdf: number; tx_count: number }>;
}
...
export function summarizePayments(rows: MomoRow[]): MomoSummary {
  ...
  const netByCategory: Record<string, { net_cdf: number; tx_count: number }> = {};
  for (const cat of CATEGORIES) {
    byCategory7d[cat] = new Array(7).fill(0);
    netByCategory[cat] = { net_cdf: 0, tx_count: 0 };
  }
